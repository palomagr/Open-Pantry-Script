...
public class Height {
    ...
    final static List<Double> startHeights = new ArrayList<>();
    final static List<Double> twoToThree = new ArrayList<>();
    final static List<Double> threeToFour = new ArrayList<>();
    final static List<Double> fourToFive = new ArrayList<>();
    final static List<Double> fiveToSix = new ArrayList<>();
    final static List<Double> sixToSeven = new ArrayList<>();

    public static void main(String[] args) {
        try {
            System.out.println("Head Height Data: ");
            String rawHeadDataPath = "src/Hospital_Data/OP head wo 0 output/OP head wo 0 organized.txt";
            //            String rawHeadDataPath = "src/HeadTextData/head OpenPantry_3_Paloma.txt";
//            File organizedFile = organizeRawHeadData(rawHeadDataPath);
            List<UserHeightTimestamp>[] processedData = processOrganizedHeadData(rawHeadDataPath);
            //                        List<PointInTime>[] processedData = processOrganizedData(new File(".\\src\\text files\\com 02-12 to 02-17 organized.txt"), rawDataPath);
            List<Double> heightFound = getHeadHeight(processedData, rawHeadDataPath);
            System.out.println("Computed user heights from head: " + heightFound);
            //            System.out.println(heightFound);
//            System.out.println("\nNow for skeleton height data:");
//            String rawSkelDataPath = "src/height_test_data/SkeletonTextData/skeleton OpenPantry_3_leo.txt";
//            //            String rawSkelDataPath = "src/SkeletonTextData/skeleton OpenPantry_3_Paloma.txt";
...
//            System.out.println("Computed height from detected joints:" + skelHeightFound);
    ...
    /**
     * Calculate the height of hte user from the caculated heights over the timespan of detection
     * @param processedData the calculated heights of the user at different times
     * @param rawDataPath the 
     * @return the calculated height of the user
     */
    public static double getSkeletonHeight(final List<UserHeightTimestamp> processedData) {
        ...
        double allTotalNumber = processedData.size();
        ...
        for (UserHeightTimestamp location: processedData) {
            ...
            } else if (currentHeight >= 1.75 && currentHeight < 2) {
            ...
            for(int i = 0; i < ID_DELIMS.length; i++) {
                if(line.contains(ID_DELIMS[i])) {
                    ...
                    Point3D point = new Point3D(Double.parseDouble(scan.next()), 
                            ...
                            Double.parseDouble(scan.next()));
                    // add the user, read height, and the time stamp to the list
        ...
        for(List<UserHeightTimestamp> singleUserData : processedData) {
            ...
            if(!singleUserData.isEmpty()) {
                ...
                for(int i = 1; i < singleUserData.size(); i++) {
                    ...
                    double currentHeight = singleUserData.get(i).getHeight();
                    double prevHeight = singleUserData.get(i-1).getHeight();
